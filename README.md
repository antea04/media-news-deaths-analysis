...
Control execution behavior with command-line flags:

```bash
# Use cached results (skip API queries)
uv run media-deaths ca_ES --cache

# Run single-keyword queries (1 mention per article)
uv run media-deaths ca_ES --single-queries

# Don't overwrite existing files
uv run media-deaths ca_ES --no-overwrite

# Use saved results file
uv run media-deaths ca_ES --use-saved-results

# Custom API sleep time (default: 10 seconds)
uv run media-deaths ca_ES --api-sleep 15

# Enable/disable verbose output
uv run media-deaths ca_ES --verbose
uv run media-deaths ca_ES --quiet
```
...
**Performance**: Time varies by config (~0.5 min per query). Example: 3 causes × 4 sources = 12 queries = ~6 min. Cached runs: <1 min (use `--cache`)
...
**Note**: Runtime settings (verbose, cache, etc.) are controlled via command-line flags, not config file.
...
**Config Reference**: See full examples in [`configs/ca_ES.yml`](configs/ca_ES.yml)

**Performance**: Use `--cache` to use cached data. Time estimate shown at start (formula: ~0.5 min × causes × sources)
